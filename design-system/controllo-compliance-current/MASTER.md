...
**Status:** implemented from the approved 2026-09-07 page plan. Treat the six-section composition as the current review baseline; freeze it after final responsive approval.

The dedicated `/platform/continuous-compliance` route uses six full-width bands: a light product-led hero, a between-audits change ledger with standards context, a dark five-step operating loop, a light Secura/readiness oversight workspace, a compact framework-reuse band, and a restrained navy conversion. Keep **Beyond the audit** teal while the full heading retains the same Manrope typography. The hero's code-built **Compliance Current workspace** shows framework scope, one control, support states, ownership, and the next review using qualitative representative data. Do not use the existing full dashboard asset, personal account data, readiness percentages, customer metrics, or integration states on this route.

The between-audits section uses four hairline-separated scenarios rather than floating cards, followed by factual SOC 2 Type II and ISO/IEC 27001 context. The operating loop uses the five manually selected steps **Set scope**, **Maintain controls**, **Add evidence**, **Review context**, and **Track readiness** around one stable product panel. Its static return cue communicates recurrence without autoplay or ambient looping. The oversight section keeps two manual tabs, **Review with Secura** and **Track readiness**. Secura is explicitly user initiated, checks selected control context, prepares a recommendation, and requires human review before any change is accepted. Readiness uses separate qualitative implementation, policy-and-procedure, evidence, and ownership signals without an aggregate score.

The framework band shows eight representative paths, but only published detail routes are interactive. Keep **Explore frameworks** as the single directory action and avoid implying that the public directory exposes every framework in the approved 100+ library. The closing conversion uses the shared trial fallback and `/demo` route over one static connected field. Do not introduce **Built by Auditors**, internal-chat functionality, autonomous compliance, continuous control testing, real-time compliance, dead framework links, invented performance evidence, videos, or decorative loops. Both selectors support pointer, touch, Arrow keys, Home, and End; reduced motion renders complete selected states immediately.
