...
Treat further hero copy, composition, product-state inventory, animation choreography, capability-rail styling, annotation positioning, and visual restyling as out of scope unless a new request explicitly reopens this hero. Accessibility corrections, verified responsive defects, product-claim corrections, metadata issues, and release-critical performance fixes may still be patched without reopening the design.

### Frozen Continuous Compliance Between Audits Baseline

**Status:** approved and locked on 2026-09-08.

Preserve the left-led **Your last audit can't tell you what changed this week** narrative, right-side operating-context lede, four ordered and hairline-separated change scenarios, distinct Lucide icons, and the factual standards context for **SOC 2 Type 2** and **ISO/IEC 27001**. Supporting explanations use readable 14px text, ordered scenarios retain direct list-item semantics, and small teal text on mist uses the contrast-safe Teal Ink token. Entrances use the shared reveal with a section-local 450ms duration and 50ms scenario stagger; reduced motion renders the complete section immediately.

Preserve the approved wave-to-content spacing, two-column desktop ledger, single-column mobile flow, border-led structure, standards band, copy hierarchy, and transition into the operating loop. Do not replace the ledger with cards, add interactive controls, introduce decorative ambient motion, or broaden the standards claims. Treat further copy, composition, iconography, spacing, animation, and visual restyling as out of scope unless a new request explicitly reopens this section. Accessibility corrections, verified responsive defects, claim corrections, and release-critical performance fixes may still be patched without reopening the design.

### Frozen Continuous Compliance Operating Loop Baseline

**Status:** approved and locked on 2026-09-08 after product-language, interaction, motion, and responsive review.

The operating loop uses the five steps **Set scope**, **Maintain controls**, **Add evidence**, **Review context**, and **Track readiness** around one stable product panel. Its viewport-filling pinned stage anchors an editorial active-chapter navigator to the left shell edge and the bounded workspace to the right. The large active number, step name, explanation, compact five-position selector, and next-step cue replace the former five-row timeline and keep meaningful section context visible during scroll. On motion-enabled viewports at least 1081px wide and 640px tall, deliberate page scroll advances those five states; pointer and keyboard selection remain available. Smaller and reduced-motion layouts stay unpinned and manual. The navy field uses a low-contrast moving square grid with one restrained fine-pointer hover cell; it never intercepts controls, touch layouts omit hover, offscreen work pauses, and reduced motion renders a static field. The background never advances the operating steps on a timer.

Preserve the claim-audited product sequence: select a framework, scope its requirements, work with mapped controls, assign accountable control owners, maintain implementation and linked policy context, keep evidence with its control and review period, review connected control and risk context, and expose qualitative readiness status plus the next accountable action. Product states remain representative and qualitative; do not add percentages, completion scores, real-time or autonomous-control claims, continuous testing, invented identifiers, or implied automatic approval. Treat further copy, chapter structure, workspace composition, grid treatment, scroll choreography, responsive layout, and visual restyling as out of scope unless a new request explicitly reopens this section. Accessibility corrections, verified responsive defects, product-claim corrections, and release-critical performance fixes may still proceed without reopening the wider design.

### Frozen Continuous Compliance Oversight Baseline

**Status:** approved and locked on 2026-09-09 after final connector, icon, copy, interaction, and responsive review.

Preserve the integrated review workspace with two manual tabs, **Review with Secura** and **Track readiness**. The section introduction stays outside the workspace; the workspace itself uses a compact narrative column and a product workflow plane without nested dashboard cards, heavy left borders, aggregate meters, or persistent background animation.

The Secura view remains explicitly user initiated. It shows selected control context, three distinct review inputs, a clear input-to-finding-to-recommendation path, the finding, the corrected **Link the latest approved access-review record** recommendation, and the boundary that human review is required before any change is accepted. Directional connectors must originate from their source plane and terminate cleanly at the destination plane without overlapping content. The readiness view uses qualitative implementation, policy-and-procedure, evidence, and ownership rows, with state labels and icons instead of percentages, scores, bars, or implied automatic readiness.

Treat further copy, iconography, connector geometry, layout, tab behavior, qualitative state inventory, and visual restyling as out of scope unless a new request explicitly reopens this section. Accessibility corrections, verified responsive defects, claim corrections, and release-critical performance fixes may still proceed without reopening the wider design.

### Frozen Continuous Compliance Framework Reuse Baseline

**Status:** approved and locked on 2026-09-09 after final product-copy, route, responsive-layout, and visual review.

Preserve the shared-control narrative, the four support signals for implementation, policy and procedure, evidence, and risk context, and the eight representative framework paths. Only SOC 2 and ISO/IEC 27001 use published detail links; all other framework names remain plain text, and **Explore frameworks** remains the single directory action. Do not imply that the public directory currently exposes every framework in the approved 100+ library.
...
**Status:** all six AI Governance sections are approved and locked for now after the framework-context approval on 2026-09-07. The full page can still receive launch-critical accessibility, claim, route, metadata, and responsive fixes.

The dedicated `/solutions/ai-governance` route uses six ordered full-width bands: the Operational AI Governance hero and connected dossier, scroll-led challenge-to-response story, AI inventory and risk-assessment workflow, Secura AI control review, framework context, and the closing AI Governance, Connected conversion. Keep the hero product-led and distinct from the Cybersecurity hero. In its headline, **AI standards** is teal while all text retains the same Manrope display typography; do not introduce italic or secondary-font emphasis. The hero canvas uses the visible functional label **AI governance workspace** with the quiet supporting line **Systems, ownership, risk, and frameworks in one view.** without presentation-only “Illustrative product view,” “Governance current,” or “Readiness lens” chrome; its figure remains accessibly named as a representative workflow. Keep the two header lines tightly grouped while the context pill stays independently aligned. Operations and Secura examples likewise use functional object labels while their qualitative example states remain claim-safe. Preserve the challenge-before-response mobile order and keep the shared header and footer behavior unchanged.

The operational governance journey uses a light field and four scroll stages: **Map**, **Assign & assess**, **Connect**, and **Review**. Keep the wide two-column section introduction above a three-layer desktop story: a narrow sticky progress rail, a substantial narrative column containing every verified challenge and response, and a compact sticky product canvas that changes with the active stage. The canvas must stay shorter and narrower than the combined narrative area, avoid unused vertical space, and show only inventory, ownership and risk, connected control context, or review content relevant to the current stage. Normal scrolling drives the change without requiring clicks. At and below 1080px, remove the sticky rail and shared canvas, then place the corresponding static canvas directly after each stage narrative. Use Phosphor icons, `overflow-x-clip` containment that preserves sticky positioning, one-time GSAP entrance motion, and complete reduced-motion output.

The operations section uses a light, non-scroll product workspace: a compact left operating-flow card supports two manual workspace views for **AI Systems** and **AI Risk Assessment**. The visual should feel connected and useful, not dashboard-heavy or dark; keep the workflow card shorter than the product workspace, use Phosphor icons, preserve the system-to-risk handoff, and avoid empty vertical space. The active record card uses a restrained moving mint perimeter current instead of a static top hairline; it is decorative, pointer-inert, and becomes static when motion is paused or reduced. The operations selector remains a manual tab system with click, arrow-key, Home, and End support. The approved framework section uses a lighter two-part lens: framework tabs on the left, one larger full-card line-field panel on the right, visible framework-context chips, and the single real `/frameworks` CTA inside the main lens. Do not reintroduce the removed right-side operating-layer/timeline card, the extra horizontal panel rule, or selected-tab box shadows. The framework selector may auto-preview its tabs only while the section is visible and motion is enabled; it must stop permanently on pointer, focus, click, or keyboard interaction, never steal focus, and remain fully manual under reduced motion. Focus remains visibly outlined. The Secura section is an approved dark chapter with compact left copy, a capability flow, and a right-side orbit-only brand visual centered on the Secura/Controllo mark. It keeps the review inputs, finding, recommended action, and human decision available semantically while avoiding weak packet icons, generated sprites, fake dashboard cards, visible "illustrative" labels, dead CTAs, and unsupported timing or autonomy claims. Framework copy describes relevant context without implying exact mappings. The closing conversion shares the Cybersecurity CTA's single-viewport conversion hierarchy while retaining its own code-built AI governance current: five system-context nodes, three restrained connected paths, traveling mint current bands, and a central mint field sit behind the approved copy and Phosphor action icons. Do not add pulse rings or large orbital rings. Ambient transforms and opacity changes run only while the section is visible; reduced motion renders the complete static field. Do not substitute the Cybersecurity film or unsupported AI performance and trust claims.

Responsive behavior is implemented at the established 1440, 1024, 768, and 375 pixel checks with contained dossiers, tabs, and proof text. Reduced motion renders every section immediately in its settled state with no required transition or loop. Evidence-dependent additions remain deferred in `FS-016`.
