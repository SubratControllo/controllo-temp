...
- Platform pages for overview, cloud monitoring, and Secura AI
...
Keep the five-section order: verified representative risk hero, compact challenge-to-response ledger, manually selected risk contexts, register/heatmap visibility, and the distinct final conversion stage. The heatmap is the focus of its selected panel without support-card or metric-rail clutter; the closing stage shows illustrative inherent and target exposure rather than claiming completed remediation. Preserve keyboard-operable tabs, reduced-motion output, and claim-safe trial/demo handoff.
