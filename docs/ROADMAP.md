...
- The Cybersecurity cloud and workforce section now uses a compact **Operational Signal Board** with a stable `Connected sources → Visible signals → Attention needed` reading path. Its Cloud Assets, Identities & Devices, and Alerts & Exposure views use product-verified labels and the exact locally vendored theSVG marks registered for AWS, Microsoft Azure, Google Cloud, Microsoft Intune, Microsoft Defender, and Google Workspace. The transition is one-shot, reduced-motion safe, and does not imply that monitoring changes control status. Keyboard navigation wraps correctly; the 375px selector exposes all three views without horizontal page overflow. Focused validation is recorded with the section implementation rather than a production build.
- `npm test -- --run src/sections/cybersecurity/CyberCloudSection.test.jsx src/pages/CybersecurityPage.test.jsx --reporter=dot`: 6 focused tests passed. Browser QA covered all three views, keyboard wrapping, exact registered logo paths, stable desktop framing, and responsive layouts at 1440px, 1024px, 768px, and 375px with zero page overflow. The production build and full test suite were not run.
