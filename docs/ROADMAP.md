...
- The explicitly reopened Cybersecurity hero now uses one controlled editorial contrast: **Cyber readiness** is Cormorant Garamond medium italic in teal at the approved `1.16em` optical scale, while the remaining headline stays Manrope medium in navy. The shared title renderer safely falls back to the complete original title when an edited accent phrase no longer matches, preventing duplicated or missing writer-provided copy.
- `npm test -- --run src/components/HeroEditorialText.test.jsx src/sections/cybersecurity/CyberHeroSection.test.jsx --reporter=dot`: 13 focused typography, fallback, hero, and interaction tests passed. Browser QA at 1440×900 and 375×812 confirmed the approved hierarchy, deliberate mobile wrapping, zero horizontal overflow, and no console warnings or errors. The Cybersecurity hero is re-approved and frozen at this state. The production build and full test suite were not run.
...
The approved baseline keeps **Cyber readiness** as the sole teal Cormorant Garamond italic headline accent at `1.16em`, with the remaining headline in navy Manrope, and preserves the two actions beside one stable-perspective **Access review** workspace. One low-contrast orbit and two same-direction Controllo emblems sit behind the dashboard; four desktop proof cards pop once from the dashboard origin, while small mobile screens retain the two primary cards. The visual remains pointer-inert, claim-safe, responsive, and complete under reduced motion.
