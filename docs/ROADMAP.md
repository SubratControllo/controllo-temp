...
- The Cybersecurity closing conversion band now uses a calmer navy-teal depth field with its visual energy concentrated on the right. Its **Kinetic Brand Mosaic** keeps the exact official emblem dominant inside twenty-one translucent white, mint-soft, and mint glass fragments. A localized halo, organic mist, subtle grain, and grounded shadow create depth without connector geometry, visible labels, unsupported metrics, or another dashboard. GSAP assembles the mosaic on viewport entry, pauses all timelines offscreen, and limits the ambient loop to slow transform and opacity displacement; reduced motion exposes the complete composition statically. The left content and both conversion buttons retain their existing classes and interaction treatment. Focused component tests pass 7/7, and responsive browser QA at 1440×1000 and 390×844 confirms zero horizontal overflow and no console warnings or errors. This section is approved and frozen on 2026-09-03; future refinement is tracked in `FS-015`. No component package or dependency was added.
