...
| `ui-skills` MCP | A UI task would benefit from discovering external UI skills by name or pulling their instructions | Use only as reference input. Adapt any retrieved skill guidance to Controllo's design system, accessibility rules, dependency policy, and frozen-surface gates. |
