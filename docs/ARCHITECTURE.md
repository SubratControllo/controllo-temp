...
The operating loop remains pointer- and keyboard-selectable with roving focus, and on motion-enabled viewports at least 1081px wide and 640px tall it maps deliberate page scroll across its five states while pinning one viewport-filling stage beneath the header. The stage pairs an editorial active-chapter navigator on the left with one stable workspace on the right. The active chapter preserves section context after the large heading leaves the viewport, and the compact five-position selector replaces the former five-row timeline. This is user-driven progression, not time-based autoplay. Smaller and reduced-motion layouts remain unpinned and manual. Its decorative square-grid canvas drifts slowly and provides one local fine-pointer hover response without intercepting the controls; touch layouts omit hover, offscreen work pauses, and reduced motion renders the field statically. The approved oversight selector changes only through pointer or keyboard input and uses one integrated workspace plane: the Secura view routes distinct review inputs to a finding and human-reviewed recommendation, while the readiness view uses qualitative support rows instead of percentages or aggregate meters. Product examples use qualitative representative states rather than customer metrics or implied live feeds. Secura remains user initiated, recommendations require accountable human review, and framework names receive detail links only when their public routes are published. The locked hero reuses the shared three-layer `WaveDivider` between its mist field and the white between-audits section, while its local `data-motion` state stops the wave animation whenever site motion is paused or reduced. Wave-to-content transitions use the wave itself as the boundary: the front layer overlaps the receiving section by 6px to suppress rendering seams, and the receiving narrative begins with the AI Governance spacing contract (`24px`, then `32px` from the medium breakpoint) instead of the generic section top padding. Do not add a separate border, rule, or spacer below a wave. `MotionContext` limits other motion to short entrances, user-driven state transitions, and the operating loop's decorative square-grid field; reduced motion renders the complete selected state immediately. Evidence-dependent product media and company claims remain governed by `FS-018` in `docs/FUTURE_SCOPE.md`.
