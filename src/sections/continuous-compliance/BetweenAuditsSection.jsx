...
export default function BetweenAuditsSection({ content, motionEnabled }) {
  return (
    <section aria-labelledby="between-audits-title" className="section bg-white">
      <div className="shell">
        <Reveal className="grid grid-cols-[minmax(0,1fr)_minmax(20rem,.68fr)] items-end gap-20 max-[900px]:grid-cols-1 max-[900px]:gap-7" motionEnabled={motionEnabled}>
          <div>
        ...
        <ol className="continuous-change-list" aria-label="Changes between audits">
          {content.scenarios.map((scenario, index) => {
            ...
            return (
              <Reveal className="continuous-change-list__item" delay={index * 0.05} key={scenario.id} motionEnabled={motionEnabled}>
                <li>
        ...
        <Reveal className="continuous-standards" delay={0.08} motionEnabled={motionEnabled}>
